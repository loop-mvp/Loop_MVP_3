...
export default function NarrativeImpact({ narrativePeriod, performance }) {
  const items = [
    ...
    ["Customer Conversions", performance.conversions ?? performance.signups],
  ];

  return (
    <div style={{ background: C.card, border: `1px solid ${C.border}`, borderRadius: 24, padding: 24 }}>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 18, marginBottom: 18, alignItems: "flex-start" }}>
        <div>
          ...
          <div style={{ marginTop: 8, fontSize: 14, lineHeight: 1.65, color: C.muted }}>
            Business outcomes tied to the current narrative period.
          </div>
        </div>
